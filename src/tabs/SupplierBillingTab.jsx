...
export default function ProductsSupplierBillingTab({ editingBillId, onSaved, onCancelEdit }) {
  ...
  async function loadData() {
    const [supplierRows, products] = await Promise.all([
  ...
  useEffect(() => {
    loadData();
    if (editingBillId) {
      window.api.supplierBills.get(editingBillId).then((bill) => {
        if (bill) {
  ...
  async function handleSaveBill() {
    ...
    setSaving(true);
    const result = await window.api.supplierBills.save({
      id: editingBillId,
      supplierId,
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">{editingBillId ? 'Edit Bill' : 'SupplierBilling'}</h1>
      </div>

      <div className="section-block billing-supplier-select">
        <div className="field">
          ...
          <Select
            ...
            onChange={(opt) => setSupplierId(opt ? opt.value : null)}
