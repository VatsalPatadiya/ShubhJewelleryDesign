...
export default function ProductsArtisanBillingTab({ editingBillId, onSaved, onCancelEdit }) {
  ...
  async function loadData() {
    const [artisanRows, products] = await Promise.all([
      window.api.artisans.list(),
  ...
  useEffect(() => {
    loadData();
    if (editingBillId) {
      window.api.artisanBills.get(editingBillId).then((bill) => {
        if (bill) {
          setArtisanId(bill.artisanId);
  ...
  async function handleSaveBill() {
    if (!artisanId) {
      showToast('Please select a artisan.', 'error');
      return;
    }
    ...
    setSaving(true);
    const result = await window.api.artisanBills.save({
      id: editingBillId,
      artisanId,
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">{editingBillId ? 'Edit Bill' : 'ArtisanBilling'}</h1>
      </div>

      <div className="section-block billing-artisan-select">
        <div className="field">
          <label>Artisan</label>
          <Select
            styles={selectStyles}
            options={artisanOptions}
            value={artisanOptions.find((o) => o.value === artisanId) || null}
            onChange={(opt) => setArtisanId(opt ? opt.value : null)}
