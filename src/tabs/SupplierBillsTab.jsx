...
import SupplierBillsTable from '../components/SupplierBillsTable.jsx';
import Modal from '../components/Modal.jsx';
import { useToast } from '../context/ToastContext.jsx';
import CustomSelect from '../components/CustomSelect.jsx';
import { formatCurrency } from '../config.js';

export default function SupplierSupplierBillsTab({ initialSupplierId, onFilterConsumed, onEditBill }) {
  ...
  async function refresh() {
    setLoading(true);
    const rows = await window.api.supplierBills.list({
      supplierId: supplierId || null,
      status,
    });
    setSupplierBills(rows);
    setLoading(false);
  }
  ...
  useEffect(() => {
    if (billToSettle) {
      const remaining = Math.round((billToSettle.grandTotal - (billToSettle.paidAmount || 0)) * 100) / 100;
  ...
  async function handleToggleStatus(bill) {
    ...
    await window.api.supplierBills.updateStatus(bill.id, nextStatus);
    setSupplierBills((prev) => prev.map((b) => (b.id === bill.id ? { ...b, status: nextStatus } : b)));
  }
  ...
  async function handleDeleteConfirmed() {
    ...
    const result = await window.api.supplierBills.delete(billToDelete);
  ...
  async function handleSettleSubmit(e) {
    ...
    const result = await window.api.supplierBills.updatePaidAmount(
      billToSettle.id, 
      amountNum, 
      paymentMethod, 
  ...
  async function handleEditSettlementSubmit(e) {
    ...
    if (isNaN(amountNum) || amountNum <= 0) {
      setEditSettlementError('Please enter a valid amount greater than 0.');
      return;
    }
    const otherPaymentsSum = (billForSettlementAction.paidAmount || 0) - settlementToEdit.amount;
    const remaining = Math.round((billForSettlementAction.grandTotal - otherPaymentsSum) * 100) / 100;
    if (amountNum > remaining) {
      setEditSettlementError(`Payment amount cannot exceed Remaining Balance of ${formatCurrency(remaining)}.`);
      return;
    }

    const result = await window.api.supplierBills.updateSettlement(
      settlementToEdit.id, 
      amountNum, 
      editPaymentMethod, 
  ...
  async function handleDeleteSettlementConfirmed() {
    ...
    const result = await window.api.supplierBills.deleteSettlement(settlementToDelete.id);
    if (result && result.success) {
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">SupplierBills</h1>
      </div>
      ...
      {!loading && supplierBills.length === 0 && (
        <div className="empty-state">
          <p>No supplierBills yet.</p>
        </div>
      )}
      ...
      {billToSettle && (
        <Modal
          title={`Settle Bill - ${billToSettle.supplierName}`}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div>
                ...
                <div style={{ fontWeight: '500', marginTop: '4px', color: 'var(--danger)' }}>
                  {formatCurrency(Math.max(0, Math.round((billToSettle.grandTotal - (billToSettle.paidAmount || 0)) * 100) / 100))}
                </div>
              </div>
            </div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (val === '') {
                    ...
                    setEditSettlementError('Payment amount must be greater than 0.');
                  } else if (num > remaining) {
                    setEditSettlementError(`Payment amount cannot exceed Remaining Balance of ${formatCurrency(remaining)}`);
                  } else {
                    setEditSettlementError('');
